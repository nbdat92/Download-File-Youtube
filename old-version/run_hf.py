...
def parse_cli(argv: List[str]) -> Tuple[Dict[str, str], List[str]]:
    ...
    Dùng tối thiểu để không phức tạp cú pháp: --hf-repo-id, --hf-branch, --hf-repo-type, --hf-path-prefix, --cookies
    """
    ov: Dict[str, str] = {}
    urls: List[str] = []
    i = 1
...
def print_help_and_exit():
    print(rf"""
YouTube -> Hugging Face Uploader  (config-first)
...
def resolve_new_files(before: set) -> List[Path]:
    now = set(DOWNLOAD_DIR.glob("*"))
    news = [p for p in now - before if p.is_file()]
    news.sort(key=lambda p: p.stat().st_mtime)
    return news
...
def run_pipeline(urls: List[str], mode: str, cfg: dict):
    ...
    with YoutubeDL(ydl_opts) as ydl:
        for i, url in enumerate(urls, 1):
            ...
            new_files = resolve_new_files(before)
            if not new_files:
                print("⚠️  Không thấy file mới.")
                continue
